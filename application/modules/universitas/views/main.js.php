<script type="text/javascript">
    $(document).ready(function() {
        var _key = "";
        var _section = "universitas";
        var _table = "table-universitas";
        var _modal = "modal-form-universitas";
        var _form = "form-universitas";

        if ($("#" + _table)[0]) {
            var table_universitas = $("#" + _table).DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "<?php echo base_url('universitas/ajax_get_all/') ?>",
                    type: "get"
                },
                columns: [
                    ...
                    { data: "nilai", render: function(data) { return data ? data : '-'; } },
        ...
        $("#" + _table).on("click", "a.action-edit", function(e) {
            e.preventDefault();
            resetForm();
            var temp = table_universitas.row($(this).closest('tr')).data();
            _key = temp.id;
        ...
        $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('universitas/ajax_save/') ?>" + _key,
                data: formData,
                processData: false,
                contentType: false,
        ...
        $("#" + _table).on("click", "a.action-delete", function(e) {
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: "delete",
                        url: "<?php echo base_url('universitas/ajax_delete/') ?>" + temp.id,
                        dataType: "json",
                        success: function(response) {
                            if (response.status) {
                                $("#" + _table).DataTable().ajax.reload(null, false);
        ...
        resetForm = () => {
            _key = "";
            $(`#${_form}`).trigger("reset");
            $(`#${_form} .universitas-is_active`).prop('checked', true);
        };
    });
</script>
