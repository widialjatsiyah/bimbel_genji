...
<body>
	...
	<div class="summary">
		...
		<p><strong>Nilai:</strong> <?= number_format($user_tryout->total_score, 2) ?></p>
	...
	<?php foreach ($questions as $index => $question): ?>
		<div class="question-block">
			...
			<?php
			// Temukan soal saat ini dari daftar soal
			...
			<?php if ($current_question && $current_question->question_type === 'essay'): 
				// var_dump($answers_essay[12]->answer_text, $question->question_id);
				?>
				<!-- Tampilkan jawaban essay -->
				<?php if (isset($answers_essay[$question->question_id]) && !empty($answers_essay[$question->question_id]->answer_text)): ?>
					<div class="essay-answer">
						<strong>Jawaban:</strong><br>
						<?= nl2br(htmlspecialchars($answers_essay[$question->question_id]->answer_text)) ?>
						
						<?php if (isset($answers_essay[$question->question_id]) && !empty($answers_essay[$question->question_id]->score)): ?>
							<div class="essay-score">Skor: <?= $answers_essay[$question->question_id]->score ?></div>
						<?php endif; ?>
					</div>
				<?php else: ?>
					<div class="essay-answer">
			...
			<?php else: ?>
				...
				<div class="options">
					...
					foreach ($options as $key => $value):
						if (!empty($value)):
							$isSelected = isset($answers[$question->id]) && !empty($answers[$question->id]->answer) && $answers[$question->id]->answer === $key;
