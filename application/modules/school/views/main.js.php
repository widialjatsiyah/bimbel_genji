<script type="text/javascript">
    $(document).ready(function() {
        ...
        var _key = "";
        var _section = "school";
        var _table = "table-school";
        var _modal = "modal-form-school";
        var _form = "form-school";

        // Initialize DataTables
        if ($("#" + _table)[0]) {
            var table_school = $("#" + _table).DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "<?php echo base_url('school/ajax_get_all/') ?>",
                    type: "get"
                },
                columns: [{
                        data: null,
                    ...
                    },
                    {
                        data: "logo_url",
                        render: function(data) {
                            return data ? '<a href="' + data + '" target="_blank">Lihat Logo</a>' : '-';
                        }
                    },
                    {
                        data: null,
        ...
        $("#" + _table).on("click", "a.action-edit", function(e) {
            e.preventDefault();
            resetForm();
            var temp = table_school.row($(this).closest('tr')).data();
        ...
        $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('school/ajax_save/') ?>" + _key,
        ...
        $("#" + _table).on("click", "a.action-delete", function(e) {
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: "delete",
                        url: "<?php echo base_url('school/ajax_delete/') ?>" + temp.id,
                        dataType: "json",
                        success: function(response) {
                            if (response.status) {
                                $("#" + _table).DataTable().ajax.reload(null, false);
