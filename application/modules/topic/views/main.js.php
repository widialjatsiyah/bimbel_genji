<script type="text/javascript">
    $(document).ready(function() {
        ...
        var _key = "";
        var _section = "topic";
        var _table = "table-topic";
        var _modal = "modal-form-topic";
        var _form = "form-topic";

        // Initialize DataTables
        if ($("#" + _table)[0]) {
            var table_topic = $("#" + _table).DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "<?php echo base_url('topic/ajax_get_all/') ?>",
                    type: "get"
                },
                columns: [{
                        data: null,
        ...
        $("#" + _table).on("click", "a.action-edit", function(e) {
            e.preventDefault();
            resetForm();
            var temp = table_topic.row($(this).closest('tr')).data();

            _key = temp.id;

            // Set nilai form
            $(`#${_form} .topic-chapter_id`).val(temp.chapter_id).trigger('change');
            $(`#${_form} .topic-name`).val(temp.name);
            $(`#${_form} .topic-order_num`).val(temp.order_num);
        });
        ...
        $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('topic/ajax_save/') ?>" + _key,
        ...
        $("#" + _table).on("click", "a.action-delete", function(e) {
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: "delete",
                        url: "<?php echo base_url('topic/ajax_delete/') ?>" + temp.id,
                        dataType: "json",
                        success: function(response) {
                            if (response.status) {
                                $("#" + _table).DataTable().ajax.reload(null, false);
        ...
        resetForm = () => {
            _key = "";
            $(`#${_form}`).trigger("reset");
            $(`#${_form} .topic-chapter_id`).val('').trigger('change');
        };
