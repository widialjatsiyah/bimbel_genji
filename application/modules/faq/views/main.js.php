<script type="text/javascript">
    $(document).ready(function() {
        ...
        var _key = "";
        var _section = "faq";
        var _table = "table-faq";
        var _modal = "modal-form-faq";
        var _form = "form-faq";

        // Initialize DataTables
        if ($("#" + _table)[0]) {
            var table_faq = $("#" + _table).DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "<?php echo base_url('faq/ajax_get_all/') ?>",
                    type: "get"
                },
                columns: [{
                        data: null,
                    ...
                    },
                    {
                        data: "question",
                        render: function(data) {
                            return data.substr(0, 50) + (data.length > 50 ? '...' : '');
                        }
                    },
                    {
                        data: "answer",
        ...
        $("#" + _table).on("click", "a.action-edit", function(e) {
            e.preventDefault();
            resetForm();
            var temp = table_faq.row($(this).closest('tr')).data();

            _key = temp.id;

            $(`#${_form} .faq-category`).val(temp.category);
            $(`#${_form} .faq-question`).val(temp.question);
            $(`#${_form} .faq-answer`).val(temp.answer);
            $(`#${_form} .faq-order_num`).val(temp.order_num);
            $(`#${_form} .faq-is_active`).prop('checked', temp.is_active == '1');
        });
        ...
        $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('faq/ajax_save/') ?>" + _key,
        ...
        $("#" + _table).on("click", "a.action-delete", function(e) {
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: "delete",
                        url: "<?php echo base_url('faq/ajax_delete/') ?>" + temp.id,
                        dataType: "json",
                        success: function(response) {
                            if (response.status) {
                                $("#" + _table).DataTable().ajax.reload(null, false);
        ...
        resetForm = () => {
            _key = "";
            $(`#${_form}`).trigger("reset");
            $(`#${_form} .faq-is_active`).prop('checked', true);
        };
