<script type="text/javascript">
  $(document).ready(function() {
    ...
    var _key = "";
    var _section = "example";
    var _table = "table-example";
    var _modal = "modal-form-example";
    var _form = "form-example";

    // Initialize DataTables: Index
    if ($("#" + _table)[0]) {
      var table_example = $("#" + _table).DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('example/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
    ...
    $("#" + _table).on("click", "a.action-edit", function(e) {
      e.preventDefault();
      resetForm();
      var temp = table_example.row($(this).closest('tr')).data();
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('example/ajax_save/') ?>" + _key,
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('example/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
    ...
    resetForm = () => {
      _key = "";
      $(`#${_form}`).trigger("reset");
      $(`#${_form} .${_section}-input_combobox`).val("").trigger('change'); // tambahkan baris ini untuk mengosongkan komponen select2
    };
