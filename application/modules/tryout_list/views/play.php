	...
	<script>
		$(document).ready(function() {
			...
			function handleQuestionTimeout() {
				...
				if (currentIndex < totalQuestions - 1) {
					...
					if (confirm('Waktu untuk soal terakhir telah habis. Apakah Anda ingin menyelesaikan sesi ini?')) {
						window.location.href = '<?= base_url("tryout_list/submit_session/") ?>' + userTryoutId + '/' + sessionId;
					}
				}
			...
			$(document).on('input propertychange paste', '[id^="essay-answer-"]', function() {
				...
				$.ajax({
					url: '<?= base_url("tryout_list/ajax_save_essay_answer") ?>',
					method: 'POST',
					data: {
			...
			$(document).on('click', '.option-item', function() {
				...
				$.ajax({
					url: '<?= base_url("tryout_list/ajax_save_answer") ?>',
					method: 'POST',
					data: {
						user_tryout_id: userTryoutId,
						question_id: qid,
			...
			$('#btn-unsure').click(function() {
				...
				$.ajax({
					url: '<?= base_url("tryout_list/ajax_mark_unsure") ?>',
					method: 'POST',
					data: {
						user_tryout_id: userTryoutId,
						question_id: qid,
			...
			$('#btn-bookmark').click(function() {
				...
				$.ajax({
					url: '<?= base_url("tryout_list/toggle_bookmark") ?>',
					method: 'POST',
					data: {
						user_tryout_id: userTryoutId,
						question_id: qid
		...
		function handleSessionTimeout() {
			...
			$.post('<?= base_url("tryout_list/complete/" . $user_tryout->id) ?>', function(response) {
				setTimeout(function() {
					window.location.href = '<?= base_url("tryout_list/result/" . $user_tryout->id) ?>';
				}, 2000);
			});
		}
		...
		function checkSessionExpired() {
			$.ajax({
				url: '<?= base_url("tryout_list/ajax_check_session_expired/" . $user_tryout->id) ?>',
				type: 'GET',
				dataType: 'json',
